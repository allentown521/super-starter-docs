...
SuperStarter defaults to using [pnpm](https://pnpm.io/installation) as a package manager, but you can use [npm](https://www.npmjs.com/), [yarn](https://yarnpkg.com/) or [bun](https://bun.sh/) instead by passing a flag during the [installation](/setup/installation/clone#use-the-cli) step.
...
SuperStarter relies on various SaaS products. You will need to create accounts with the following services then set the API keys in your [environment variables](/setup/configuration/environment):
