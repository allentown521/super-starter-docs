...
- environment variables with the correct values for your email service.

## Internationalization

SuperStarter supports multiple languages and allows to switch between them with a language switcher. Per default `English` , `German` ,  `Spanish` , `Chinese` , `French` are provided and enabled.

A common pitfall is to keep the language enabled that you don't want to support, which can unwanted behavior as the site will automatically redirect the user to the language that fits the browser language. If you don't want to support a language, make sure to disable it in the `config.ts` file in the `languageData` section.
...
<Note>Although we use the default English even if the translation is incomplete, you should make sure that all languages ​​have translations.</Note>
...
If you have set up billing for your app, you need to set up [webhooks](/packages/payments#webhooks) for the payment provider to sync the purchases to your database.
...
Read the guides on [metadata](/packages/seo/metadata) and [sitemap](/packages/seo/sitemap) to learn more.
...
SuperStarter has defined a few placeholder pages for legal pages, like privacy policy which you can edit in the `/apps/web/content/legal/*.md` files.
...
Make sure that all the [environment variables](/setup/configuration/environment) that are required for your app to work are set in your deployment environment.
