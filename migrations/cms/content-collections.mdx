...
```sh Terminal
pnpm add @content-collections/mdx  --filter @repo/cms
...
<Note>We're remapping the `title` field to `_title` and the `_meta.path` field to `_slug` to match the default SuperStarter CMS.</Note>
