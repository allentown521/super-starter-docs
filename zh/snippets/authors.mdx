export const Authors = ({ data }) => {
  const baseUrl = (typeof window !== 'undefined' && window.location.origin.includes('localhost')) ? '' : 'https://raw.githubusercontent.com/focusapps/ShipThing/refs/heads/main/docs';
