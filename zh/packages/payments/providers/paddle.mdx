---
title: Paddle
description: 了解如何使用 SuperStarter 设置 Creem。
---
...
在为 Creem 创建账户后，您需要获取 API 密钥。您可以通过访问仪表盘上的 [API 页面](https://www.paddle.com/) 来完成此操作。在这里，您将找到密钥和可发布密钥。您需要使用密钥来完成集成。

### 添加环境变量

要使用 Creem 集成，您需要在 `.env.local` 文件以及生产环境中定义以下环境变量：
...
删除 `apps/web/api/paddle/checkout/route.ts` 中的注释。
