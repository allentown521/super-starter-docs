---
title: 数据库
description: SuperStarter 中数据库和 ORM 的配置方式。
---
...
SuperStarter 旨在提供一个健壮、类型安全的数据库客户端，使您能够轻松地使用数据库，同时在整个应用程序中保持强类型。我们支持以下工具：
...
默认情况下，SuperStarter 使用 [Neon](https://neon.tech) 作为数据库提供商，使用 [Prisma](https://prisma.io) 作为 ORM。但是，如果您愿意，可以轻松切换到其他提供商，例如：
...
SuperStarter 包含 Prisma Studio，这是一个可视化的数据库编辑器。要启动它，请运行以下命令：
