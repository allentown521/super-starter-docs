---
title: Configuration
descriptions: Configure billing for your application.
---
...
<Tip>
`priceId` from the provider (e.g. Stripe) or `variantId` (e.g. Lemon Squeezy) , some providers use `productId` instead of `priceId`. Any way you can use `priceId` to store it.
</Tip>